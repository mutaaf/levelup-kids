...
2026-06-17 [human/escalation] CI unit-tests RLS suite died with `permission denied for table quest_completions` on the service-role client → `supabase/setup-cli@v1 + supabase start` does NOT apply Supabase's "default privileges" to public.* tables, so a migration that only CREATEs tables + ENABLEs RLS leaves every role (including service_role) with zero grants → every migration touching public.* must end with explicit `grant select, insert, update, delete on all tables in schema public to authenticated, service_role` (and the matching sequence usage grant). RLS still enforces per-row gating for `authenticated`; service_role bypasses RLS by design.
