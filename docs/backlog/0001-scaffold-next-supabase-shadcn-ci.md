...
- 2026-06-16 [ship/0001] Shipped. Next.js pinned to `^15.1.4` (the scaffolder default `create-next-app@latest` produced Next 16 which is out of scope; downgraded the `package.json` deps to match the ticket's Next 15 / React 19 brief). Body face is **Public Sans** via `next/font/google` — the documented open fallback because Söhne is licensed (`docs/DESIGN.md §3`); swap-out is a single line in `src/app/layout.tsx` when a Söhne license lands. Fraunces (variable, opsz axis) + JetBrains Mono are both `next/font/google`. All three are exposed as Tailwind v4 theme vars (`--font-display`, `--font-body`, `--font-mono`) and bound via the CSS variable names `--font-fraunces` / `--font-body` / `--font-jetbrains-mono` so `next/font`'s class injection works without a CDN URL. shadcn primitives (button, card, dialog, input, label, badge) written in-tree to avoid the interactive `shadcn init` prompt; primary button is `bg-brand-500` (terracotta) — no blue default. Supabase scaffolded via `supabase init` (project_id `levelup-kids`, port 54321); `supabase/seed.sql` is a `SELECT 1 WHERE FALSE` no-op so `psql -v ON_ERROR_STOP=1` is a green no-op in CI. CI workflow has three jobs named `lint` / `unit-tests` / `e2e-tests` (exact gating-check names per `AGENTS.md § Agent parameters`); `e2e-tests` boots `supabase start` via `supabase/setup-cli@v1`, captures URL + keys into `$GITHUB_ENV` with `supabase status -o env --override-name …`, applies `supabase/seed.sql` via `psql`, builds Next, then runs Playwright for both `chromium` and `mobile-webkit` projects. Local gate green: `npm run lint && npm run typecheck && npm run test && node scripts/check-backlog.mjs`; `npm run test:e2e` also green on both projects.
